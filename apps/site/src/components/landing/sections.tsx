...
import { Badge, Button, Rating, ThemeController } from "@wizeworks/silicaui-react";
import { InstallCommand } from "./install-command";
import { ElementCard } from "./element-card";
import { PropsDemo } from "./props-demo";
import { Reveal } from "./reveal";
import { BEHAVIOR_COUNT, COMPONENT_COUNT, PACKAGE_COUNT } from "@/lib/counts";
    ...
    >
      <div className="mx-auto flex w-full max-w-6xl items-center justify-between gap-4 px-6 py-3.5">
        ...
        <div className="flex items-center gap-4">
          ...
          {/* The site sells theming, so it has to be themeable itself. Writes an
              explicit `data-theme` on <html>, which outranks the plugin's
              prefers-dark rule — OS decides the first paint, this decides after
              that. See docs/personas/issues/002. */}
    ...
    <section data-theme="dark" className="relative overflow-hidden bg-base-100">
      <div className="relative mx-auto grid w-full max-w-6xl grid-cols-1 gap-12 px-6 pb-24 pt-20 md:pb-28 md:pt-24 lg:grid-cols-[1fr_1.15fr] lg:items-center lg:gap-14">
        <div className="flex flex-col items-start gap-6">
          ...
          {/* `w-full min-w-0`: in the column direction this wrapper would otherwise
              take the width of its widest child, and the install command's command
              text is wider than a 360px screen. See docs/personas/issues/004. */}
...
/* ---------------------------------------------------------------------------
   ...
   Derived, never typed. These were hand-written string literals under a comment
   claiming they were "checked against the repo" — nothing checked them, the repo
   grew, and the front page ended up printing 113 components in this band and 116
   in the next section, twelve lines below. See docs/personas/issues/001.

   `@/lib/counts` is generated by apps/site/scripts/gen-counts.mjs before every
   build, and `pnpm verify` fails if it has drifted from the repo.
   --------------------------------------------------------------------------- */

const STATS: { value: string; label: string }[] = [
  { value: String(COMPONENT_COUNT), label: "Documented components" },
  { value: String(BEHAVIOR_COUNT), label: "Vanilla behaviors" },
  { value: String(PACKAGE_COUNT), label: "Published packages" },
      ...
      <div className="mx-auto w-full max-w-6xl px-6 py-24 md:py-28">
        <Reveal>
          <h2 className="max-w-2xl text-4xl font-semibold tracking-tight text-base-content md:text-5xl">
            {components.length} components across {PACKAGE_COUNT} packages
          </h2>
        </Reveal>
        <Reveal delay={80}>
          <p className="mt-4 max-w-2xl text-lg text-base-content">
            You install four of them. The core carries no third-party runtime; every heavy
            dependency lives in an opt-in sibling you add only when you reach for it.
          </p>
        </Reveal>
        ...
        {/* The number, made real: every one of them is a live, linked doc page.
            A claim becomes something a visitor can scan and click. */}
  ...
  return (
    ...
    <section className="bg-primary">
      <div className="mx-auto flex w-full max-w-3xl flex-col items-center gap-7 px-6 py-32 text-center">
        ...
        {/* `w-full min-w-0` on BOTH the Reveal and the row: this column is
            `items-center`, so every box in the chain sizes to its content, and the
            second <InstallCommand />'s text is wider than a 360px screen. Constraining
            only the inner row leaves the Reveal itself overflowing.
            See docs/personas/issues/004. */}
        <Reveal delay={140} className="w-full min-w-0">
          <div className="flex w-full min-w-0 flex-col items-center gap-3 pt-2 sm:flex-row">
            <InstallCommand />
