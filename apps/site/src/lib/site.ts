...
/**
 * Order matters, and the middle entry is the one that was missing:
 *
 *   1. `CURATED` — a hand-written marketing sentence, where one beats the extracted
 *      prose. Fifteen of these exist and they stay authoritative.
 *   2. `CATALOG` — the real description from the MCP catalog the repo already ships,
 *      generated by scripts/gen-catalog.mjs. This covers 114 of 116.
 *   3. `templateDescription` — the name in a stock sentence. It used to serve 101 of
 *      116 pages and is now a genuine last resort, reached by `Hooks` and
 *      `Resizable Panels` only.
 *
 * See docs/personas/issues/006: a reader comparing Table with Data Table got one real
 * sentence and one template, so the comparison could not be made.
 */
export function componentDescription(id: string, title: string): string {
  return CURATED[id] ?? CATALOG[id]?.description ?? templateDescription(title);
}
