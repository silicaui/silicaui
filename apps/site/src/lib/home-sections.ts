...
import { BEHAVIOR_COUNT, COMPONENT_COUNT, PACKAGE_COUNT } from "./counts";
...
/**
 * Counts derived from the repo at build time, never typed.
 *
 * These were the literals `"113"` and `"34"` under a comment saying they were
 * "read off this repo, not invented". Nothing read them off anything, and both
 * had gone stale — the same two wrong numbers the front page was printing. See
 * docs/personas/issues/001. `./counts` is generated by
 * apps/site/scripts/gen-counts.mjs and checked by `pnpm verify`.
 */
    ...
    el("section", "@container bg-base-100", {
      children: [
        el("div", "mx-auto grid w-full max-w-5xl grid-cols-2 gap-8 px-6 py-16 @2xl:grid-cols-4", {
          children: [
            stat(String(COMPONENT_COUNT), "Components"),
            stat(String(BEHAVIOR_COUNT), "Behaviors"),
            stat(String(PACKAGE_COUNT), "Packages"),
            stat("MIT", "License"),
          ],
        }),
      ],
